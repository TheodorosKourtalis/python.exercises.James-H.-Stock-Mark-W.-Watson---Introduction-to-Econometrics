...
Select an exercise and work through the interactive inputs. Click **Show Sample Answer** to compare your solution.
""")
...
# Global helper: show sample answer with PDF generation for small screens.
...
def show_sample_answer(sample_md, key_suffix="default"):
    # Αν το st.session_state["small_screen"] είναι True, δίνεται η PDF έκδοση.
...
# Εάν η μεταβλητή δεν έχει οριστεί (σε standalone λειτουργία)
...
def exercise_2_1():
    ...
    st.markdown("""
**Question:**  
Give one example each of a discrete random variable and a continuous random variable from everyday life, and explain why they belong to these categories.
*Hint:* Countable (π.χ. αριθμός τηλεφωνικών κλήσεων) vs. μετρήσιμο (π.χ. χρόνος μετακίνησης).
    """)
    st.text_area("Your Answer:", height=150, key="ex2_1")
    with st.expander("Show Sample Answer"):
        st.markdown("""
**Sample Answer:**  
- **Discrete:** Ο αριθμός των email που λαμβάνεις σε μία μέρα (μόνο ακέραιοι αριθμοί).  
- **Continuous:** Ο χρόνος που χρειάζεσαι για το ταξίδι σου (π.χ. 35.27 λεπτά, μπορεί να έχει απεριόριστη ακρίβεια).
        """)
